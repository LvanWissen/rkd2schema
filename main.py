...
def getEventLabel(event):
    ...
    if event.hasTimeStamp:
        ...
        if (
            str(event.hasEarliestBeginTimeStamp)[:4]
            == str(event.hasLatestEndTimeStamp)[:4]
        ):
    ...
    if persons:
        labelNL = Literal(
            f"{eventNameNL} van {' en '.join(persons)} ({year})", lang="nl"
        )
        labelEN = Literal(
            f"{eventNameEN} of {' and '.join(persons)} ({year})", lang="en"
        )
    else:
    ...
    if r["response"]["numFound"] > 1:
        ...
        while start < nTotal:
            ...
            try:
                ...
                print(f"Error fetching {url}. Trying again.")
                sleep(1)
                continue
            ...
            if doc is None:
                r = requests.get(APIURL + identifier, params={"format": "json"}).json()
                doc = r["response"]["docs"][0]
...
def parseData(d, thesaurusDict=dict()):
    ...
    for k in keywordIdentifiers:
        concept, thesaurusDict = getThesaurus(k, thesaurusDict, returnType="uri")
        keywords.append(concept)

    attributedTo = [
        {"identifier": i["naam_linkref"], "name": i["naam_inverted"]}
    ...
    for r in d["onderdeel_van"]:
        try:
            ...
            related.append(
                {
                    "relatedTo": URIRef(
                        f"https://rkd.nl/explore/images/{relIdentifier}"
                    ),
                    "relation": r.get("onderdeel_van_verband"),
                }
            )
        except KeyError:
            pass
    ...
    for r in d["artistiek"]:
        ...
        related.append(
            {
                "relatedTo": URIRef(f"https://rkd.nl/explore/images/{relIdentifier}")
                if relIdentifier
                else None,
                "relation": r.get("onderdeel_van_verband"),
                "description": ", ".join(
                ...
                "relationThesaurus": r.get("artistiek_verband_linkref"),
            }
        )
    ...
    for p in d["voorgestelde"]:
            ...
            marriages.append(
                {
                    ...
                    "marriagePartnerNameWithIdentifier": m.get(
                        "naam_huw_partner_samenvoeging"
                    ),
                    "marriagePlace": m.get("huwelijk_plaats"),
                    "marriagePlaceIdentifier": m.get(
                        "huwelijk_plaats_lref"
                    ),  # mapping TGN?
                }
            )
        ...
        if len(aetatisSet) == 1:
            ...
            print(f"Multiple aetatis values for image {identifier}: {aetatisSet}")
            aetatis = [Literal(str(a["age"])) for a in abouts if a["age"] is not None]
        ...
        if p.get("vader"):
            ...
            father = Person(
                nsPerson.term(str(fatherIdentifier)), name=[fatherName], children=[pURI]
            )
            parents.append(father)
        elif p.get("naam_vader"):
            father = Person(
                unique(p["persoonsnummer"], p["naam_vader"]),
                name=[p["naam_vader"]],
                children=[pURI],
            )
            parents.append(father)
        ...
        if p.get("moeder"):
            ...
            mother = Person(
                nsPerson.term(str(motherIdentifier)), name=[motherName], children=[pURI]
            )
            parents.append(mother)
        elif p.get("naam_moeder"):
            mother = Person(
                unique(p["persoonsnummer"], p["naam_moeder"]),
                name=[p["naam_moeder"]],
                children=[pURI],
            )
            parents.append(mother)
        ...
        if p.get("kinderen"):
            for k in p["kinderen"]:
                ...
                child = Person(
                    nsPerson.term(str(childIdentifier)), name=[childName], parent=[pURI]
                )
    ...
    for a in attributedTo:
        ...
        exploreArtistURI = URIRef(f"https://rkd.nl/explore/artists/{a['identifier']}")
...
def getQuantitativeValue(value):
    ...
    if value and value != "?":
        ...
        try:
            ...
            qv = QuantitativeValue(None, unitCode="MTR", value=literal, label=label)
...
def getThesaurus(
    ...
    for i in cachedict[identifier]["broader"]:
        broader, cachedict = getThesaurus(
            i, cachedict, returnType=returnType, recursionDepth=recursionDepth
        )
    ...
    for i in cachedict[identifier]["narrower"]:
        narrower, cachedict = getThesaurus(
            i, cachedict, returnType=returnType, recursionDepth=recursionDepth
        )
        if narrower:
    ...
    for i in cachedict[identifier]["related"]:
        related, cachedict = getThesaurus(
            i, cachedict, returnType=returnType, recursionDepth=recursionDepth
        )
        if related:
    ...
    if returnType == "uri":
        ...
        concept = Concept(
            ...
            if cachedict[identifier]["descriptionNL"]
            else [],
            related=relateds,
            broader=broaders,
            narrower=narrowers,
...
def parseThesaurusURL(identifier, url):
    ...
    if (
        "No description available" in description
        or "Geen beschrijving beschikbaar" in description
    ):
        description = None
    ...
    for broaderElement in broaderElements:
        if (
            "Broader term" in broaderElement.text
            or "Ruimere term" in broaderElement.text
        ):  # since class labels are screwed up
            broaderUrls = [e.attrs["href"] for e in broaderElement.findAll("a")]
    ...
    for narrowElement in narrowElements:
        if (
            "Narrower term" in narrowElement.text
            or "Nauwere term" in narrowElement.text
        ):
            narrowerUrls = [e.attrs["href"] for e in narrowElement.findAll("a")]
            narrowers += [i.split("term=")[1] for i in narrowerUrls]
        if (
            "Related term" in narrowElement.text
            or "Verwante term" in narrowElement.text
        ):  # since class labels are screwed up
...
def getPerson(p):
    ...
    person = Person(
        nsPerson.term(str(p["identifier"])),
        name=[p["name"]],
        disambiguatingDescription=[Literal(p["disambiguatingDescription"], lang="nl")]
        if p["disambiguatingDescription"]
        else [],
    )
    ...
    if p["birthPlace"] or p["birthDateBegin"] or p["birthDateEnd"]:
        ...
        birth = Birth(
            nsPerson.term(str(p["identifier"] + "#birth")),
            principal=person,
            place=place,
    ...
    if p["baptismDateBegin"] or p["baptismDateEnd"]:
        ...
        baptism = Baptism(
            nsPerson.term(str(p["identifier"] + "#baptism")),
            principal=person,
            hasTimeStamp=timeStamp,
            hasEarliestBeginTimeStamp=earliestBeginTimeStamp,
            hasLatestEndTimeStamp=latestEndTimeStamp,
    ...
    if p["deathPlace"] or p["deathDateBegin"] or p["deathDateEnd"]:
        ...
        death = Death(
            nsPerson.term(str(p["identifier"] + "#death")),
            principal=person,
            place=place,
            hasTimeStamp=timeStamp,
            hasEarliestBeginTimeStamp=earliestBeginTimeStamp,
    ...
    if p["burialDate"]:
        ...
        burial = Burial(
            nsPerson.term(str(p["identifier"] + "#burial")),
            principal=person,
            hasTimeStamp=date,
        )
    ...
    for m in p["marriages"]:
        if (
            m.get("marriagePlace")
            or m.get("marriageDate")
            or m.get("marriagePartner")
            or m.get("marriagePartnerIdentifier")
        ):

            place = getPlace(m["marriagePlaceIdentifier"], [m["marriagePlace"]])
            ...
            if m["marriagePartnerIdentifier"]:
                if m["marriagePartnerNameWithIdentifier"]:
                    ...
                    name = []
                partner = Person(
                    nsPerson.term(str(m["marriagePartnerIdentifier"])), name=name
                )
            elif m["marriagePartner"]:
                partner = Person(
                    unique(p["identifier"], m["marriagePartner"]),
            ...
            marriage = Marriage(
                unique(set([p["identifier"], m["marriagePartner"], "marriage"])),
                place=place,
                hasTimeStamp=date,
                partner=partners,
            )
...
def main(
    ...
    ## Serialize
    print("Serializing!")
    ds.serialize("rkdportraits14001850.trig", format="trig")
